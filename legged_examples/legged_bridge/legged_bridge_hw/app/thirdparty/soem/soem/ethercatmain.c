...
#ifdef EC_VER1
/** Main slave data array. 主要从机数据数组
 *  Each slave found on the network gets its own record.  网络上的每一个从机都有自己的记录
 *  ec_slave[0] is reserved for the master.  ec_slave[0] 是主站数据    
 *  Structure gets filled in by the configuration function ec_config().  结构由 ec_config() 配置
 */
 ...
 * @return Work counter.
 */
int ecx_receive_processdata_group(ecx_contextt *context, uint8 group, int timeout)  //用于接受ethercan从站的过程数据组
{
   int pos, idx;
   int wkc = 0, wkc2;
   uint16 le_wkc = 0;
      ...
      if (wkc2 > EC_NOFRAME)
      {
         if((context->port->rxbuf[idx][EC_CMDOFFSET]==EC_CMD_LRD) || (context->port->rxbuf[idx][EC_CMDOFFSET]==EC_CMD_LRW))
         {
            if(first)
            {
               ...
               *(context->DCtime) = etohll(le_DCtime);  //etohll() 用于将小端字节序的 64 位无符号整数转换为主机字节序 对输入字节进行适当的重排，以使其在主机中正确解释
                                                        //使时间值在主机环境中正确解释和处理
               first = FALSE;
            }
            else
